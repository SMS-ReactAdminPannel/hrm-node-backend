...
export const EmployeeGetAll = async (req, res) => {
  try {
    const Employee = await employeeUser
      .find({ is_active: true })
      .populate({path: "details_id", model:"EmployeeDetail", select:"-password"})
      .populate({path:"department",model:"departmentmodels"});
...
export const EmployeeCreate = async (req, res) => {
  try {
    ...
    const passwordHash = await bcrypt.hash(password, 10);
    const employeeCount = await employeeUser.countDocuments();
    const nextEmployeeId = `EMP${String(employeeCount + 1).padStart(3, "0")}`;

    const newEmployee = new employeeUser({
      employeeUser_id: nextEmployeeId,
      first_name,
      last_name,
      email,
      phone_number,
